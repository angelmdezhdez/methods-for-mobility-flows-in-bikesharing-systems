...
if __name__ == '__main__':
    ...
    for index in indexes:
        current_weight = alpha[index].cpu().numpy()
        current_flow = flows[index]
        plt.figure(figsize=(12, 5))
        plt.subplot(1, 2, 1)
        plt.imshow(current_flow, cmap='viridis', aspect='auto')
        plt.title(f'Flow {index}')
        plt.colorbar()
        plt.subplot(1, 2, 2)
        plt.imshow(dictionary @ current_weight, cmap='viridis', aspect='auto')
